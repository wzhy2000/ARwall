...
bool MyGLCamera::CheckModelMat(glm::mat4 mMat) {
    if ( isnan( mMat[0][0] ) || isnan( mMat[1][1]) || isnan( mMat[2][2]) )
        return false;

    CV_Assert(!isnan( mMat[0][0] ));
    CV_Assert(!isnan( mMat[1][1] ));
    CV_Assert(!isnan( mMat[2][2] ));

    return true;
}
/*
 * Projects the set of 2D points provided onto 3D space at an assumed distance from wall
 * ref_points -> 2D points, x,y as per CV
 * distFromWall -> Vertical distance of camera from the wall
 * Returns a vector of back-projected 3D points (w.r.t OpenGL origin)
 */
std::vector<cv::Point3f> MyGLCamera::GetProjectedPointsOnWall(std::vector<cv::Point2f> sourceKeypoints,
    ...
    for(unsigned int i=0; i < sourceKeypoints.size(); i++){
        ...
        glm::vec3 pointOnWall = unitRay * (distFromWall / cosineRayGravity) +
                                 cameraPosition;

        pointLocationsIn3D[i].x = pointOnWall.x;
        pointLocationsIn3D[i].y = pointOnWall.y;
        pointLocationsIn3D[i].z = pointOnWall.z;
...
void MyGLCamera::UpdateModelMat(cv::Mat translationVector,
    ...
    modelDefaultTranslation.convertTo(modelDefaultTranslation, CV_32F);
    translateModel[3][0] = modelDefaultTranslation.at<float>(0, 0);
    translateModel[3][1] = modelDefaultTranslation.at<float>(1, 0);
    translateModel[3][2] = modelDefaultTranslation.at<float>(2, 0);

    if (CheckModelMat(modelMat)) {
